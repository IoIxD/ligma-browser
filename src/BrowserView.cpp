...
void BrowserView::DisplayHandler::OnLoadingProgressChange(
    CefRefPtr<CefBrowser> browser,
    double progress) {
...
BrowserView::BrowserView(const std::string& url)
  ...
  m_client = new BrowserClient(m_render_handler, m_display_handler);
...
//------------------------------------------------------------------------------
void BrowserView::mouseClick(CefBrowserHost::MouseButtonType btn,
                             bool mouse_up) {
  CefMouseEvent evt;
  evt.x = m_mouse_x;
  evt.y = m_mouse_y;
