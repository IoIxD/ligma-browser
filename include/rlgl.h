 ...
 *
 *   rlgl is a wrapper for multiple OpenGL versions (1.1, 2.1, 3.3 Core, ES 2.0)
 *to pseudo-OpenGL 1.1 style functions (rlVertex, rlTranslate, rlRotate...).
 *
 *   When chosing an OpenGL version greater than OpenGL 1.1, rlgl stores vertex
 *data on internal VBO buffers (and VAOs if available). It requires calling 3
 *functions: rlglInit()  - Initialize internal buffers and auxiliary resources
 *       rlglClose() - De-initialize internal buffers data and other auxiliar
 *resources
 *
 *   CONFIGURATION:
 *
 ...
 *       Those preprocessor defines are only used on rlgl module, if OpenGL
...
#define RLAPI // We are building or using rlgl as a static library (or Linux
              // shared library)

#if defined(_WIN32)
#if defined(BUILD_LIBTYPE_SHARED)
#define RLAPI                                                                  \
  __declspec(dllexport) // We are building raylib as a Win32 shared library
                        // (.dll)
...
#define MAX_BATCH_ACTIVE_TEXTURES                                              \
  4 // Maximum number of additional textures that can be activated on batch
...
typedef struct VertexBuffer {
  ...
  int vCounter;  // Vertex position counter to process (and draw) from full
                 // buffer
  int tcCounter; // Vertex texcoord counter to process (and draw) from full
                 // buffer
...
typedef struct DrawCall {
  ...
  // Shader id to be used on the draw -> Using RLGL.currentShader.id
...
RLAPI void
rlRotatef(float angleDeg, float x, float y,
...
RLAPI void
rlMultMatrixf(float *matf); // Multiply the current matrix by another matrix
...
RLAPI void rlLoadExtensions(
    void *loader); // Load OpenGL extensions (loader function pointer required)
...
// are required
RLAPI RenderBatch rlLoadRenderBatch(
...
RLAPI void rlDrawRenderBatch(
    RenderBatch *batch); // Draw render batch data (Update->Draw->Reset)
RLAPI void
rlSetRenderBatchActive(RenderBatch *batch); // Set the active render batch for
...
RLAPI unsigned int
rlLoadVertexBuffer(void *buffer, int size,
                   bool dynamic); // Load a vertex buffer attribute
RLAPI unsigned int
rlLoadVertexBufferElement(void *buffer, int size,
...
rlLoadTextureDepth(int width, int height,
                   bool useRenderBuffer); // Load depth texture/renderbuffer (to
                                          // be attached to fbo)
RLAPI unsigned int rlLoadTextureCubemap(void *data, int size,
...
RLAPI void rlGenerateMipmaps(
    Texture2D *texture); // Generate mipmap data for selected texture
...
#define DEFAULT_SHADER_ATTRIB_NAME_NORMAL                                      \
  "vertexNormal" // Binded by default to shader location: 2
#endif
#ifndef DEFAULT_SHADER_ATTRIB_NAME_COLOR
#define DEFAULT_SHADER_ATTRIB_NAME_COLOR                                       \
  "vertexColor" // Binded by default to shader location: 3
#endif
...
typedef struct rlglData {
  ...
  struct {
    ...
    unsigned int
        activeTextureId[MAX_BATCH_ACTIVE_TEXTURES]; // Active texture ids to be
                                                    // enabled on batch drawing
                                                    // (0 active by default)
    ...
    Shader
        defaultShader; // Basic shader, support vertex color and diffuse texture
    Shader currentShader; // Shader to be used on rendering (by default,
                          // defaultShader)
  ...
  struct {
    ...
    bool texDepth;   // Depth textures supported (GL_ARB_depth_texture,
                     // GL_WEBGL_depth_texture, GL_OES_depth_texture)
...
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
static void
rlLoadShaderDefault(void); // Load default shader (RLGL.State.defaultShader)
static void
rlUnloadShaderDefault(void); // Unload default shader (RLGL.State.defaultShader)
...
#if defined(GRAPHICS_API_OPENGL_11)
static int
rlGenerateMipmapsData(unsigned char *data, int baseWidth,
                      int baseHeight); // Generate mipmaps data on CPU side
static Color *
rlGenNextMipmapData(Color *srcData, int srcWidth,
...
void rlTranslatef(float x, float y, float z) {
  ...
  *RLGL.State.currentMatrix =
      MatrixMultiply(matTranslation, *RLGL.State.currentMatrix);
}
...
void rlRotatef(float angleDeg, float x, float y, float z) {
  ...
  // NOTE: We transpose matrix with multiplication order
  *RLGL.State.currentMatrix =
      MatrixMultiply(matRotation, *RLGL.State.currentMatrix);
}
...
void rlScalef(float x, float y, float z) {
  ...
  // NOTE: We transpose matrix with multiplication order
  *RLGL.State.currentMatrix =
      MatrixMultiply(matScale, *RLGL.State.currentMatrix);
}
...
void rlFrustum(double left, double right, double bottom, double top,
  ...
  *RLGL.State.currentMatrix =
      MatrixMultiply(*RLGL.State.currentMatrix, matPerps);
}
...
void rlOrtho(double left, double right, double bottom, double top, double znear,
  ...
  // a division by zero on MatrixOrtho(), response to it is platform/compiler
  // dependant
  Matrix matOrtho = MatrixOrtho(left, right, bottom, top, znear, zfar);

  *RLGL.State.currentMatrix =
      MatrixMultiply(*RLGL.State.currentMatrix, matOrtho);
}
#endif
...
void rlBegin(int mode) {
  ...
  // vertex buffer
  if (RLGL.currentBatch->draws[RLGL.currentBatch->drawsCounter - 1].mode !=
      mode) {
    if (RLGL.currentBatch->draws[RLGL.currentBatch->drawsCounter - 1]
            .vertexCount > 0) {
        ...
        RLGL.currentBatch->draws[RLGL.currentBatch->drawsCounter - 1]
            ...
            ((RLGL.currentBatch->draws[RLGL.currentBatch->drawsCounter - 1]
                 ...
                 : RLGL.currentBatch->draws[RLGL.currentBatch->drawsCounter - 1]
                           .vertexCount %
                       4);
      else if (RLGL.currentBatch->draws[RLGL.currentBatch->drawsCounter - 1]
                   .mode == RL_TRIANGLES)
        RLGL.currentBatch->draws[RLGL.currentBatch->drawsCounter - 1]
            .vertexAlignment =
            ((RLGL.currentBatch->draws[RLGL.currentBatch->drawsCounter - 1]
                 ...
                 ? 1
                 : (4 - (RLGL.currentBatch
                             ->draws[RLGL.currentBatch->drawsCounter - 1]
                             .vertexCount %
                         4)));
      else
        RLGL.currentBatch->draws[RLGL.currentBatch->drawsCounter - 1]
...
void rlEnd(void) {
  // Make sure vertexCount is the same for vertices, texcoords, colors and
  // normals NOTE: In OpenGL 1.1, one glColor call can be made for all the
  // subsequent glVertex calls

  // Make sure colors count match vertex count
  if (RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer]
          .vCounter !=
      RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer]
          .cCounter) {
    int addColors =
    ...
    for (int i = 0; i < addColors; i++) {
      RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer]
          .colors[4 * RLGL.currentBatch
                          ->vertexBuffer[RLGL.currentBatch->currentBuffer]
                          .cCounter] =
          RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer]
              .colors[4 * RLGL.currentBatch
                              ->vertexBuffer[RLGL.currentBatch->currentBuffer]
  ...
  // TODO: Make sure normals count match vertex count... if normals support is
  // added in a future... :P
  ...
  if ((RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer]
      ...
      (RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer]
           ...
           4 -
       4)) {
    // WARNING: If we are between rlPushMatrix() and rlPopMatrix() and we need
    // to force a rlDrawRenderBatch(), we need to call rlPopMatrix() before to
    // recover *RLGL.State.currentMatrix (RLGL.State.modelview) for the next
    // forced draw call! If we have multiple matrix pushed, it will require
    // "RLGL.State.stackCounter" pops before launching the draw
...
void rlglInit(int width, int height) {
  ...
  RLGL.State.defaultTextureId =
      rlLoadTexture(pixels, 1, 1, PIXELFORMAT_UNCOMPRESSED_R8G8B8A8, 1);
  ...
  RLGL.defaultBatch =
      rlLoadRenderBatch(DEFAULT_BATCH_BUFFERS, DEFAULT_BATCH_BUFFER_ELEMENTS);
...
#if !defined(__APPLE__)
#if !defined(EPOXY)
  // NOTE: With GLAD, we can check if an extension is supported using the
  // GLAD_GL_xxx booleans
  ...
  for (int i = 0; i < numExt; i++) {
        ...
        0) // Web ANGLE
    {
      glDrawArraysInstanced =
          (PFNGLDRAWARRAYSINSTANCEDEXTPROC)eglGetProcAddress(
              "glDrawArraysInstancedANGLE");
      glDrawElementsInstanced =
          (PFNGLDRAWELEMENTSINSTANCEDEXTPROC)eglGetProcAddress(
              "glDrawElementsInstancedANGLE");
      glVertexAttribDivisor =
          (PFNGLVERTEXATTRIBDIVISOREXTPROC)eglGetProcAddress(
...
// Show some basic info about GL supported features
  ...
  if (RLGL.ExtSupported.texNPOT)
    ...
    TRACELOG(LOG_WARNING, "GL: NPOT textures extension not found, limited NPOT "
                          "support (no-mipmaps, no-repeat)");
#endif
...
RenderBatch rlLoadRenderBatch(int numBuffers, int bufferElements) {
  ...
  //--------------------------------------------------------------------------------------------
  batch.vertexBuffer =
      (VertexBuffer *)RL_MALLOC(sizeof(VertexBuffer) * numBuffers);
  ...
  for (int i = 0; i < numBuffers; i++) {
    ...
    glEnableVertexAttribArray(
        RLGL.State.currentShader.locs[SHADER_LOC_VERTEX_POSITION]);
    glVertexAttribPointer(
        RLGL.State.currentShader.locs[SHADER_LOC_VERTEX_POSITION], 3, GL_FLOAT,
        0, 0, 0);
    ...
    glEnableVertexAttribArray(
        RLGL.State.currentShader.locs[SHADER_LOC_VERTEX_TEXCOORD01]);
    glVertexAttribPointer(
        RLGL.State.currentShader.locs[SHADER_LOC_VERTEX_TEXCOORD01], 2,
        GL_FLOAT, 0, 0, 0);
    ...
    glEnableVertexAttribArray(
        RLGL.State.currentShader.locs[SHADER_LOC_VERTEX_COLOR]);
    glVertexAttribPointer(
        RLGL.State.currentShader.locs[SHADER_LOC_VERTEX_COLOR], 4,
  ...
  //--------------------------------------------------------------------------------------------
  batch.draws =
      (DrawCall *)RL_MALLOC(DEFAULT_BATCH_DRAWCALLS * sizeof(DrawCall));
...
void rlDrawRenderBatch(RenderBatch *batch) {
  ...
  if (batch->vertexBuffer[batch->currentBuffer].vCounter > 0) {
    ...
    glBufferSubData(GL_ARRAY_BUFFER, 0,
                    batch->vertexBuffer[batch->currentBuffer].vCounter * 3 *
                        sizeof(float),
    ...
    glBufferSubData(GL_ARRAY_BUFFER, 0,
                    batch->vertexBuffer[batch->currentBuffer].vCounter * 2 *
                        sizeof(float),
    ...
    glBufferSubData(GL_ARRAY_BUFFER, 0,
                    batch->vertexBuffer[batch->currentBuffer].vCounter * 4 *
  ...
  for (int eye = 0; eye < eyesCount; eye++) {
    if (eyesCount == 2) {
      ...
      rlSetMatrixModelview(
          MatrixMultiply(matModelView, RLGL.State.viewOffsetStereo[eye]));
    ...
    if (batch->vertexBuffer[batch->currentBuffer].vCounter > 0) {
      ...
      Matrix matMVP =
          MatrixMultiply(RLGL.State.modelview, RLGL.State.projection);
      glUniformMatrix4fv(RLGL.State.currentShader.locs[SHADER_LOC_MATRIX_MVP],
                         1, false, MatrixToFloat(matMVP));
      ...
      if (RLGL.ExtSupported.vao)
        ...
        glBindBuffer(GL_ARRAY_BUFFER,
                     batch->vertexBuffer[batch->currentBuffer].vboId[0]);
        glVertexAttribPointer(
            RLGL.State.currentShader.locs[SHADER_LOC_VERTEX_POSITION], 3,
            GL_FLOAT, 0, 0, 0);
        glEnableVertexAttribArray(
            RLGL.State.currentShader.locs[SHADER_LOC_VERTEX_POSITION]);
      ...
      glUniform4f(RLGL.State.currentShader.locs[SHADER_LOC_COLOR_DIFFUSE], 1.0f,
                  1.0f, 1.0f, 1.0f);
      ...
      for (int i = 0, vertexOffset = 0; i < batch->drawsCounter; i++) {
        // Bind current draw call texture, activated as GL_TEXTURE0 and binded
        // to sampler2D texture0 by default
        ...
        if ((batch->draws[i].mode == RL_LINES) ||
          ...
          // quadsCount*6 NOTE: The final parameter tells the GPU the offset in
          // bytes from the start of the index buffer to the location of the
...
unsigned int rlLoadTexture(void *data, int width, int height, int format,
                           int mipmapCount) {
    ...
    if (glInternalFormat != -1) {
      if (format < PIXELFORMAT_COMPRESSED_DXT1_RGB)
        glTexImage2D(GL_TEXTURE_2D, i, glInternalFormat, mipWidth, mipHeight, 0,
                     glFormat, glType, (unsigned char *)data + mipOffset);
#if !defined(GRAPHICS_API_OPENGL_11)
      else
  ...
  if (id > 0)
    TRACELOG(
        LOG_INFO,
        "TEXTURE: [ID %i] Texture loaded successfully (%ix%i - %i mipmaps)", id,
...
// WARNING: OpenGL ES 2.0 requires GL_OES_depth_texture/WEBGL_depth_texture
// extensions
      ...
      if (data == NULL) {
        if (format < PIXELFORMAT_COMPRESSED_DXT1_RGB) {
          if (format == PIXELFORMAT_UNCOMPRESSED_R32G32B32) {
            // Instead of using a sized internal texture format (GL_RGB16F,
            // GL_RGB32F), we let the driver to choose the better format for us
            // (GL_RGB)
            if (RLGL.ExtSupported.texFloat32)
              glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB, size,
                           size, 0, GL_RGB, GL_FLOAT, NULL);
            else
              TRACELOG(LOG_WARNING,
          ...
          TRACELOG(LOG_WARNING, "TEXTURES: Empty cubemap creation does not "
...
void rlGenerateMipmaps(Texture2D *texture) {
  ...
  if (texIsPOT) {
    ...
    if (texture->format == PIXELFORMAT_UNCOMPRESSED_R8G8B8A8) {
      ...
      int mipmapCount =
          rlGenerateMipmapsData(texData, texture->width, texture->height);
      ...
      for (int level = 1; level < mipmapCount; level++) {
        glTexImage2D(GL_TEXTURE_2D, level, GL_RGBA8, mipWidth, mipHeight, 0,
      ...
      RL_FREE(texData); // Once mipmaps have been generated and data has been
                        // uploaded to GPU VRAM, we can discard RAM data
      ...
      TRACELOG(LOG_WARNING,
               "TEXTURE: [ID %i] Failed to generate mipmaps for provided "
               "texture format",
               texture->id);
  }
#endif
  ...
  if ((texIsPOT) || (RLGL.ExtSupported.texNPOT)) {
    ...
    // generation algorythm: GL_FASTEST, GL_NICEST, GL_DONT_CARE
...
#define MAX(a, b) (((a) > (b)) ? (a) : (b))
    ...
    texture->mipmaps =
        1 + (int)floor(log(MAX(texture->width, texture->height)) / log(2));
    TRACELOG(LOG_INFO,
    ...
    TRACELOG(LOG_WARNING, "TEXTURE: [ID %i] Failed to generate mipmaps",
             texture->id);
...
void *rlReadTexturePixels(Texture2D texture) {
  ...
  unsigned int size =
      rlGetPixelDataSize(texture.width, texture.height, texture.format);
...
bool rlFramebufferComplete(unsigned int id) {
  ...
  if (status != GL_FRAMEBUFFER_COMPLETE) {
    ...
    case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT:
      TRACELOG(LOG_WARNING, "FBO: [ID %i] Framebuffer has a missing attachment",
               id);
      break;
    default:
      break;
...
void rlDrawVertexArrayElementsInstanced(int offset, int count, void *buffer,
                                        int instances) {
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
  glDrawElementsInstanced(GL_TRIANGLES, count, GL_UNSIGNED_SHORT,
                          (unsigned short *)buffer + offset, instances);
#endif
}
...
unsigned int rlLoadShaderCode(const char *vsCode, const char *fsCode) {
  ...
  for (int i = 0; i < uniformCount; i++) {
    ...
    TRACELOGD("SHADER: [ID %i] Active uniform (%s) set at location: %i", id,
...
unsigned int rlCompileShader(const char *shaderCode, int type) {
  ...
  if (success == GL_FALSE) {
    ...
    switch (type) {
    case GL_VERTEX_SHADER:
      TRACELOG(LOG_INFO, "SHADER: [ID %i] Vertex shader compiled successfully",
               shader);
      break;
    case GL_FRAGMENT_SHADER:
      TRACELOG(LOG_INFO,
...
unsigned int rlLoadShaderProgram(unsigned int vShaderId,
  ...
  if (success == GL_FALSE) {
    ...
    TRACELOG(LOG_INFO, "SHADER: [ID %i] Program shader loaded successfully",
             program);
#endif
  return program;
}
...
int rlGetLocationUniform(unsigned int shaderId, const char *uniformName) {
  ...
  if (location == -1)
    TRACELOG(LOG_WARNING, "SHADER: [ID %i] Failed to find shader uniform: %s",
             shaderId, uniformName);
  else
    TRACELOG(LOG_INFO,
             "SHADER: [ID %i] Shader uniform (%s) set at location: %i",
             shaderId, uniformName, location);
#endif
  return location;
}
...
int rlGetLocationAttrib(unsigned int shaderId, const char *attribName) {
  ...
  if (location == -1)
    TRACELOG(LOG_WARNING, "SHADER: [ID %i] Failed to find shader attribute: %s",
             shaderId, attribName);
  else
    TRACELOG(LOG_INFO,
             "SHADER: [ID %i] Shader attribute (%s) set at location: %i",
             shaderId, attribName, location);
#endif
  return location;
}
...
static void rlLoadShaderDefault(void) {
  RLGL.State.defaultShader.locs =
      (int *)RL_CALLOC(MAX_SHADER_LOCATIONS, sizeof(int));
  ...
  RLGL.State.defaultVShaderId = rlCompileShader(
      vShaderDefault, GL_VERTEX_SHADER); // Compile default vertex shader
  RLGL.State.defaultFShaderId = rlCompileShader(
      fShaderDefault, GL_FRAGMENT_SHADER); // Compile default fragment shader

  RLGL.State.defaultShader.id = rlLoadShaderProgram(
...
static int rlGenerateMipmapsData(unsigned char *data, int baseWidth,
  ...
  int size = baseWidth * baseHeight *
             4; // Size in bytes (will include mipmaps...), RGBA only
  ...
  if (temp != NULL)
    ...
    TRACELOG(LOG_WARNING,
             "TEXTURE: Failed to re-allocate required mipmaps memory");
...
static Color *rlGenNextMipmapData(Color *srcData, int srcWidth, int srcHeight) {
  ...
  for (int y = 0; y < height; y++) {
    ...
    for (int x = 0; x < width; x++) {
      ...
      prow.r =
          (srcData[y2 * srcWidth + x2].r + srcData[y2 * srcWidth + x2 + 1].r) /
          2;
      prow.g =
          (srcData[y2 * srcWidth + x2].g + srcData[y2 * srcWidth + x2 + 1].g) /
          2;
      prow.b =
          (srcData[y2 * srcWidth + x2].b + srcData[y2 * srcWidth + x2 + 1].b) /
          2;
      prow.a =
          (srcData[y2 * srcWidth + x2].a + srcData[y2 * srcWidth + x2 + 1].a) /
          2;

      pcol.r = (srcData[(y2 + 1) * srcWidth + x2].r +
                srcData[(y2 + 1) * srcWidth + x2 + 1].r) /
               2;
      pcol.g = (srcData[(y2 + 1) * srcWidth + x2].g +
                srcData[(y2 + 1) * srcWidth + x2 + 1].g) /
               2;
      pcol.b = (srcData[(y2 + 1) * srcWidth + x2].b +
                srcData[(y2 + 1) * srcWidth + x2 + 1].b) /
               2;
      pcol.a = (srcData[(y2 + 1) * srcWidth + x2].a +
                srcData[(y2 + 1) * srcWidth + x2 + 1].a) /
               2;
